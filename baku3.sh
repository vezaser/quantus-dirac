...
# przyda się też curl lub wget do pobrania quantus-miner
if ! command -v curl >/dev/null 2>&1 && ! command -v wget >/dev/null 2>&1; then
  say "❌ Brak curl ani wget. W WSL zainstaluj np.: sudo apt-get install -y curl"
  exit 1
fi

say "🚀 Quantus DIRAC — node + miner w Dockerze (wersja pod WSL, bez apt-get w kontenerach)"
say "--------------------------------------------------------------------------------------"
...
read -rp "👉 Czy masz już adres do nagród (qz...)? (t/n): " HAVE_ADDR
REWARD_ADDR=""

# Zaciągnijmy obraz noda (przyda się tak czy siak)
say "⬇️  Pobieram obraz noda Quantus (v0.4.2)..."
...
# 3) Pobieramy gotowy binarek quantus-miner na hosta
...
# 4) Tworzymy prosty Dockerfile.miner NA UBUNTU (GLIBC >= 2.32), BEZ apt-get
...
say "🛠  Buduję obraz minera (local/quantus-miner:latest) na bazie Ubuntu 22.04..."
...
# Node łączy się z minerem po host.docker.internal:9833,
# a my wystawiamy port 9833 z minera na hosta.
...
services:
  quantus-node:
    ...
    command:
      ...
      - "256"
      - --external-miner-url
      - http://host.docker.internal:9833
      - --bootnodes
